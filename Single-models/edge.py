...
class master_query(Sim.Process) :
	...
	def run(self, request_query) :
		while(1) :
			...
			yield Sim.hold,self,1000 + constant.RND.expovariate(constant.QUERY_RATE)
...
class processor(Sim.Process) :
	...
	def run(self, node_no) :
		while(1) :
			...
			yield Sim.hold,self,0.5/constant.fact+constant.EDGE_PROC_RATE
			#print("check1",len(G.slave_nodes_attr[node_no].processors_queue))
	...
	def run(self, node_no) :
		while(1) :
			...
			try:
				...
				yield Sim.hold,self,0.5/constant.fact+constant.EDGE_DISK_RATE
			except:
				print("exception")
